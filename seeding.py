"""Idempotent seed data: the team's user accounts and a demo client.

Kept free of any ``app`` import so it can be called both from the ``seed.py``
CLI and from ``app.create_app`` on boot (when ``SEED_DEMO=1``) without a
circular import. Callers must provide a Flask app context.
"""
...
def seed_demo_client() -> None:
    ...
    db.session.commit()
    print("  + demo client: John & Jane Sample")
